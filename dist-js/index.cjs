...
const appWindow = new window.Window('serial-port');
let tester_ports = {};
...
class SerialPort {
    ...
    static async available_ports() {
        try {
            const result = await core.invoke('plugin:serialport|available_ports');
    ...
    async close() {
        try {
            ...
            let res = undefined;
            if (!this.is_test) {
                res = await core.invoke('plugin:serialport|close', {
                    path: this.options.path,
                });
            }
    ...
    async open() {
        try {
            ...
            if (this.is_test) {
                ...
                res = await core.invoke('plugin:serialport|open', {
                    path: this.options.path,
