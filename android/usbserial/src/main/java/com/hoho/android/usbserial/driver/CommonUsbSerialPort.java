...
public abstract class CommonUsbSerialPort implements UsbSerialPort {
  ...
  /** Retry queue — concurrent SIOM write can briefly fail queue on some hosts. */
    private boolean queueReadRequest(ByteBuffer buf, int length) {
    ...
    protected void testConnection(boolean full, String msg) throws IOException {
        ...
        if(len < 0) {
            // Unreliable on CH340/CDC under concurrent SIOM+write; see usb-serial-for-android #503.
            // Must not tear down an open port based on this heuristic.
    ...
    @Override
    public int read(final byte[] dest, final int length, final int timeout) throws IOException {return read(dest, length, timeout, false);}
    ...
    protected int read(final byte[] dest, int length, final int timeout, boolean testConnection) throws IOException {
        ...
        if (timeout != 0) {
            ...
            if (nread == -1) {
                if (testConnection && MonotonicClock.millis() < endTime) {
                    try {
                        testConnection(true);
                    } catch (IOException e) {
                        // bulkTransfer idle timeout + failed GET_STATUS is common on CDC modems
                        // after TX bursts; not a reliable disconnect signal.
                    }
                }
                return 0;
            }
            ...
            if(useReadQueue()) {
                ...
                if (!queueReadRequest(buf, length)) {
            ...
            if (nread == 0) {
                try {
                    ...
                    // Idle read with failed GET_STATUS is common on modems during concurrent
                    // SIOM write; not a reliable disconnect signal — continue listening.
                    return 0;
                }
            }
        }
    ...
    public void write(final byte[] src, int length, final int timeout) throws IOException {
        ...
        while (offset < length) {
            ...
            if (actualLength <= 0) {
                ...
                if (timeout != 0) {
                    // could be buffer full: writing too fast, flow control, or CH340 busy with SIOM read
                    if (elapsed < timeout) {
                        try {
                            testConnection(true);
                        } catch (IOException e) {
                            // bulkTransfer -1 + failed GET_STATUS is common on CDC/CH340 during
                            // concurrent SIOM; not a reliable disconnect signal — report timeout.
                        }
                    }
