...
describe('SerialPort API error matrix', () => {
  ...
  describe('Mixed Operation Tests', () => {
    ...
    it('should handle recovery after errors', async () => {
      ...
      await expect(serialPort.write('test')).rejects.toThrow('Port is not open');
  ...
  describe('Error Handling in Specific Methods', () => {
    ...
    it('should handle error in writeRequestToSend', async () => {
      mockInvoke.mockRejectedValueOnce(new Error('Set RTS error'));
      await expect(serialPort.writeRequestToSend(true)).rejects.toThrow('Set RTS error');
    });

    it('should handle error in writeDataTerminalReady', async () => {
      mockInvoke.mockRejectedValueOnce(new Error('Set DTR error'));
      await expect(serialPort.writeDataTerminalReady(true)).rejects.toThrow('Set DTR error');
    });
  ...
  describe('Edge cases', () => {
    it('should handle empty path in open', async () => {
      ...
      await expect(port.open()).rejects.toThrow('path cannot be empty');
    });

    it('should reject readBinary when port is closed', async () => {
      serialPort.isOpen = false;
      await expect(serialPort.readBinary()).rejects.toThrow('Port is not open');
    });

    it('should reject watch when port is closed', async () => {
      serialPort.isOpen = false;
      await expect(serialPort.watch({ onData: jest.fn() })).rejects.toThrow('Port is not open');
    });
