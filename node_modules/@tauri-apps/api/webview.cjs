 ...
 */
function getCurrent() {
    return new Webview(window$1.getCurrent(), window.__TAURI_INTERNALS__.metadata.currentWebview.label, {
...
function getAll() {
    ...
    new Webview(window$1.Window.getByLabel(w.windowLabel), w.label, {
        // @ts-expect-error `skip` is not defined in the public API but it is handled by the constructor
        skip: true
    }));
}
...
class Webview {
    ...
    async onFileDropEvent(handler) {
        const unlistenFileDrop = await this.listen(event.TauriEvent.WEBVIEW_FILE_DROP, (event) => {
            handler({
                ...event,
                payload: {
                    type: 'drop',
        ...
        const unlistenFileHover = await this.listen(event.TauriEvent.WEBVIEW_FILE_DROP_HOVER, (event) => {
            handler({
                ...event,
                payload: {
                    type: 'hover',
        ...
        const unlistenCancel = await this.listen(event.TauriEvent.WEBVIEW_FILE_DROP_CANCELLED, (event) => {
...
applyMixins(WebviewWindow, [window$1.Window, Webview]);
